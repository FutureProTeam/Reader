...
class DefaultServer implements Server {
    ...
    DefaultServer(int port, int timeout, Map<String, RequestHandler> handlerMap, HttpRequestHandler noMatchHttpRequestHandler, WebSite webSite, Listener listener) {
        this.mPort = port;
    ...
    public void start() {
        if (!isRunning()) {
            mCore = new CoreThread(mPort, mTimeout, mHandlerMap, noMatchHttpRequestHandler, mWebSite, mListener);
            mCore.start();
        }
    }
